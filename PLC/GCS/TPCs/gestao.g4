...
gestao: stock ';' ';' faturas[$stock.prodts]
    { System.out.println("Quantidades em Stock no fim:"); 
      System.out.println($faturas.prodout.toString()); 
    };
...
faturas[HashMap<String, Produto> prodin] returns [HashMap<String, Produto> prodout]
    @init {$faturas.prodout = new HashMap<String, Produto>(); }:
    {
    System.out.println("Quantidades em Stock no meio:"); System.out.println($faturas.prodin.toString());
    }
	f1 = fatura[$faturas.prodin] {$faturas.prodout = $f1.prodout;}
    ( ';' f2 = fatura[$faturas.prodin] {$faturas.prodout = $f2.prodout;})*
    /*{
        $faturas.prodout = $faturas.prodin;
    }*/
;

fatura[HashMap<String, Produto> prodin] returns [HashMap<String, Produto> prodout]:
	'FATURA' cabecalho 'VENDAS' corpo[$fatura.prodin] PONTO     {System.out.println("TOTAL da Fatura: "+ $corpo.totF);
                                                                    $fatura.prodout = $corpo.prodout;
                                                                    };
...
corpo[HashMap<String, Produto> prodin] returns [Double totF, HashMap<String, Produto> prodout]:
	 HIFEN l1=linha[$corpo.prodin] { $corpo.totF = $l1.totL; $corpo.prodout = $l1.prodout;} 
    ( HIFEN l2=linha[$corpo.prodin] { $corpo.totF += $l2.totL; $corpo.prodout = $l2.prodout;} )*
    /*{$corpo.prodout = $corpo.prodin;}*/;

linha[HashMap<String, Produto> prodin] returns [Double totL, HashMap<String, Produto> prodout]:

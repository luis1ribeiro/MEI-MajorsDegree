...
--}
-- Por não ter o Monad do PowerSet em haskell, usamos o Monad das Listas.
import Data.List
...
-- Using bind. Same as exec.
run = initialMove >>= possibleMoves >>= possibleMoves >>= possibleMoves >>= return
...
-- Combinação do PowerSet e write monads.
...
-- Using bind. Same as exec.
runl = linitialMove >>= lpossibleMoves >>= lpossibleMoves >>= lpossibleMoves >>= return
